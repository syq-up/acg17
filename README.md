...
- 漫画元数据管理、封面上传、ZIP 章节导入、分页阅读、收藏、分类标签筛选和回收站
...
漫画不再单独维护作者字段，艺术家统一作为分类标签管理。上传漫画和编辑漫画详情时均可选择已有标签或创建新标签；漫画列表页可按标签筛选，并优先展示默认折叠的艺术家标签。
...
列表页的标签筛选顺序为艺术家、角色、男性、女性、混合、其他、原作。选择任意标签后，前端通过 `tagId` 查询与该标签关联的漫画。
